...
class BtprintExecutor {
  ...
  void testPrint() async {
    ...
    bluetooth.isConnected.then((isConnected) async {
      if (isConnected == true) {
        ...
        await bluetooth.writeBytes(Uint8List.fromList([0x1B, 0x4C, 0x01])).then(
            (value) async => await bluetooth.write('\nAYAM CABE GARAM 1'));
        await bluetooth.write('\n Printer Normally');
        await bluetooth.writeBytes(Uint8List.fromList([0x1B, 0x21, 0x00])).then(
            (value) async => await bluetooth.write('\nAYAM CABE GARAM 3'));
    ...
    bluetooth.isConnected.then((value) async {
      if (value != true) {
        ...
        await bluetooth.printCustom(
            'PERNYATAAN', Size.boldMedium.val, Align.center.val);
        await bluetooth.writeBytes(center);
        await bluetooth.writeBytes(normal);
        await bluetooth
            .write('Saya & rekan tidak  akan membawa masuk dan atau\n');
        await bluetooth
            .write('mengkonsumsi makanan/minuman yang bukan berasal\n');
        await bluetooth.write(
            'dari outlet ${data.outlet.namaOutlet} ini. Apabila  terbukti\n');
        await bluetooth
            .write('kemudian, saya  bersedia dikenakan denda sesuai\n');
    ...
    bluetooth.isConnected.then((isConnected) async {
      if (isConnected == true) {
        ...
        if (data.voucherValue != null && (data.voucherValue?.price ?? 0) > 0) {
          await bluetooth.write(formatTable(
              'Voucher',
              '(${Formatter.formatRupiah((data.voucherValue?.price ?? 0))})',
              48));
        }
        ...
        if (isNotNullOrEmpty(data.transferList)) {
          ...
          for (var teep in data.transferList) {
            await bluetooth.write(formatTableRow(
                teep.room, Formatter.formatRupiah(teep.transferTotal), 48,
                leftSize: 33,
                rightSize: 15,
                alignRight: true,
                alignLeft: true));
        ...
        await bluetooth.printNewLine();
        await bluetooth.printCustom(
            'Rp ${Formatter.formatRupiah(data.dataInvoice.jumlahBersih)}',
            Size.boldMedium.val,
            Align.center.val);
        await bluetooth.writeBytes(normal);
        await bluetooth.printNewLine();
  ...
  void printInvoice(PrintInvoiceModel data) async {
    ...
    bluetooth.isConnected.then((isConnected) async {
      ...
      await bluetooth
          .write('------------------------------------------------\n\n');
      //Tax And Service
      await printFooter(bluetooth, data.dataInvoice, data.dataServiceTaxPercent,
          (data.footerStyle ?? 1));
      ...
      if (isNotNullOrEmpty(data.paymentList)) {
        for (var payment in data.paymentList) {
          await bluetooth.write(formatTableRow(
              payment.paymentName, Formatter.formatRupiah(payment.value), 48,
              leftSize: 33, rightSize: 15, alignRight: true, alignLeft: true));
        }
      } else {
      ...
      await bluetooth.printNewLine();
      await bluetooth.printCustom(
          'Kembali ${Formatter.formatRupiah(data.payment.payChange)}',
          Size.boldMedium.val,
          Align.right.val);
      await bluetooth.printNewLine();
      await bluetooth.printNewLine();
  ...
  Future<void> printTransfer(
      BlueThermalPrinter bluetooth, TransferModel data, int footerStyle) async {
    List<OrderFinalModel> orderFix = List.empty(growable: true);
    ...
    //Tax And Service
    await printFooter(
        bluetooth, data.dataInvoice, data.dataServiceTaxPercent, footerStyle);
  ...
  Future<void> printFooter(BlueThermalPrinter bt, InvoiceModel ivc,
    ...
    if (style == 1) {
      ...
      await bt.write(formatTableRow(
          ...
          leftSize: 33, rightSize: 15, alignRight: true, alignLeft: true));
      await bt.write(formatTableRow(
          'Jumlah Service ${(tns.serviceFnbPercent + tns.serviceRoomPercent) / 2}%',
          Formatter.formatRupiah(ivc.fnbService + ivc.roomService),
          48,
          leftSize: 33,
          rightSize: 15,
          alignRight: true,
          alignLeft: true));
      await bt.write(formatTableRow(
          'Jumlah pajak ${(tns.taxFnbPercent + tns.taxRoomPercent) / 2}%',
          Formatter.formatRupiah(ivc.fnbTax + ivc.roomTax),
          48,
          leftSize: 33,
          rightSize: 15,
          alignRight: true,
      ...
      await bt.write(formatTableRow(
          'Harga Tertera sudah termasuk service dan pajak', '', 48,
          leftSize: 33, rightSize: 15, alignRight: true, alignLeft: true));
    } else if (style == 4) {
    } else if (style == 5) {
      await bt.write(formatTableRow(
          'Jumlah', Formatter.formatRupiah(ivc.jumlah), 48,
          leftSize: 33, rightSize: 15, alignRight: true, alignLeft: true));
      await bt.write(formatTableRow('Jumlah Service',
          Formatter.formatRupiah(ivc.fnbService + ivc.roomService), 48,
          leftSize: 33, rightSize: 15, alignRight: true, alignLeft: true));
      await bt.write(formatTableRow(
          'Jumlah pajak', Formatter.formatRupiah(ivc.fnbTax + ivc.roomTax), 48,
          leftSize: 33, rightSize: 15, alignRight: true, alignLeft: true));
      await bt.writeBytes(right);
      await bt.write('----------------\n');
      await bt.writeBytes(normal);
  ...
  Future<void> printFnB(
      List<OrderFinalModel> data, BlueThermalPrinter bt) async {
    ...
    for (var fnb in fnbList) {
      if (GlobalProviders.read(showReturProvider) == true) {
        ...
        if (fnb.jumlahCancel > 0) {
          await bt.write(formatTable(
              '  RETUR ${fnb.namaItem} x ${Formatter.formatRupiah(fnb.jumlahCancel)}',
